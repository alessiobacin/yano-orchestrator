...
function reportBug(db, opts) {
	...
	const normalized = { title: opts.title.trim(), description: opts.description.trim(), expected: opts.expected || "", actual: opts.actual || "", steps: parseSteps(opts.steps), environment: safeJson(json(opts.environment, opts.environment ? { value: opts.environment } : {})) };
...
export async function runYanoDebugger({ argv = [] } = {}) {
	...
	try {
		...
		if (["claim", "transition", "promote"].includes(opts.sub)) {
			const bug = getBugOrThrow(db, opts.bugId);
			if (opts.sub === "claim") {
				if (bug.assigned_instance && bug.assigned_instance !== opts.actor) throw new Error(`yano debugger claim: bug già assegnato a ${bug.assigned_instance}`);
				db.prepare("UPDATE debugger_bugs SET assigned_instance = ?, updated_at = ? WHERE bug_id = ?").run(opts.actor, now(), bug.bug_id);
				const updated = bugWithProject(db, db.prepare("SELECT * FROM debugger_bugs WHERE bug_id = ?").get(bug.bug_id));
				appendDebuggerEvent(db, updated, "claimed", opts.actor);
		...
		if (opts.sub === "pause" || opts.sub === "resume") {
			...
			if (existing.worker_status === "running" && !opts.dryRun && !has(argv, "--force")) {
				const result = { project: info.name, worker_status: "running", already_running: true, workspace_id: existing.workspace_id, tab_id: existing.worker_tab_id, instance: existing.worker_instance };
				print(result, opts.json);
				return result;
			}
			const basePort = existing.backend_base_port || await allocateBasePort(db, opts.basePort);
			const launched = opts.foreground
				? { workspace_id: existing.workspace_id, tab_id: existing.worker_tab_id, pane_id: existing.worker_pane_id, instance: existing.worker_instance || `debugger-${info.name}`, command: null, supervisor: "foreground", dry_run: opts.dryRun }
				: launchHerdrWorker({ project: info, root: info.root, db, row: existing, intervalMs: opts.intervalMs, dryRun: opts.dryRun });
			...
			try { appendRawTraceRecord({ cwd: info.root, project: info.name, record: { type: `debugger_${opts.sub}`, record_type: "event", source: "yano-debugger", instance: "yano-debugger", worker_status: status } }); } catch { /* best effort */ }
			print(result, opts.json);
			return result;
		}
		if (opts.sub === "start") {
			...
			const launched = opts.foreground
				? { workspace_id: project.workspace_id, tab_id: project.worker_tab_id, pane_id: project.worker_pane_id, instance: project.worker_instance || `debugger-${info.name}`, command: null, supervisor: "foreground", dry_run: opts.dryRun }
				: launchHerdrWorker({ project: info, root: info.root, db, row: project, intervalMs: opts.intervalMs, dryRun: opts.dryRun });
			if (opts.foreground) db.prepare("UPDATE debugger_projects SET worker_instance = ?, worker_status = ?, interval_ms = ?, updated_at = ? WHERE project_key = ?").run(launched.instance, opts.dryRun ? "planned" : "running", opts.intervalMs, now(), project.project_key);
			const result = { project: info.name, worker_status: opts.dryRun ? "planned" : "running", ports: portsFor(basePort), ...launched };
			print(result, opts.json);
			return result;
		}
