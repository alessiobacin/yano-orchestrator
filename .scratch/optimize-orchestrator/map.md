...
- [CLI `yano`: packaging e migrazione](issues/06-cli-yano-packaging-migration.md) — il nome ufficiale è `yano-orchestrator`, il solo binario è `yano`, senza alias `po`, e la rinomina comprende riferimenti pubblici e interni.
...
- [Audit distribuzione pacchetto `yano`](issues/17-yano-package-distribution-audit.md) — audit fallito: il pacchetto espone ancora `po`, si chiama `yolo-orchestrator` e non distribuisce i Playbook sotto `.pi/`.
