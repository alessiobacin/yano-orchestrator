...
9. If the task involved frontend roles, the planner asks whether the user wants a visual development review. On consent, `yano frontend-review start` installs the required development dependencies, selects the React adapter or the Angular host adapter, infers and starts the frontend URL, and the planner consumes Agentation annotations through MCP; unknown frameworks remain browser-only until an adapter exists.
