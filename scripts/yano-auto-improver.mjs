...
async function notifyPlanner(info, audit, summary) {
	...
	const message = { type: "command", assignment_id: `auto-improve-${audit.audit_id}`, sender_instance: "yano-auto-improver", sender_role: "auto-improver", project: info.name, correlation_id: audit.audit_id, display: true, triggerTurn: true, followUp: true, prompt: `[yano-auto-improver] Audit completato per ${info.name}. Leggi il report ${audit.report_path}. Summary informativo: ${summary}. Esegui tu il triage del report e, se necessario, formula tu le domande e raccogli le decisioni dell'utente; l'auto-improver non ha modificato il progetto e non deve chiedere decisioni direttamente.` };
