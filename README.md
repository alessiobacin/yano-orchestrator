...
- **Persistent agent memory** — every LLM round updates bounded Markdown memory shared by project/role, with separate user preferences and instance diagnostics; memory survives agent kill/restart and is excluded from Git (see `docs/quick-guides/23-memoria-agenti.md`)
- **Project memory and memory CLI** — a short project summary is shared by every agent without repeating it in role memory; use `yano memory agents|list|show|create|update|delete` to inspect or manage the Markdown memories
- **Existing-project documentation bootstrap** — the first planner preflight scans manifests, structure, entrypoints and documentation before deep code exploration; it creates a short `project.md` only when missing, asks the user before invoking `docs-sync`, and lets `docs-sync` refresh documents that are present but stale. The planner verifies the result and owns the final `project.md` update.
- **Progressive context exploration** — every agent reads project/role memory and relevant documentation before code, starts from assigned files, expands only for dependencies or gaps, verifies critical facts against the real implementation and reports its evidence surface.
- **Schedule execution history** — `yano schedule instances --id <job-id> --limit N` shows bounded execution status and `yano schedule retry --id <instance-id>` retries a specific dispatch with an audit link to the original
...
`yano update` updates both places the extension can live: the global npm package (`npm install -g` against this repo's GitHub URL) and, if present, the separate clone `pi extension install` keeps under `~/.pi/agent/git/github.com/<owner>/<repo>` (a plain `git pull`). When the global package is a local `npm link`, it detects and removes only that exact symlink, then installs a real permanent npm copy; a local commit therefore cannot become active implicitly. At the end it also runs `pi update --extensions` and synchronizes the global `yano-cli` skill in the detected harness catalogs, so Pi's installed extension registry and CLI skill are synchronized before the next session. A failure in those final synchronizations is reported clearly without hiding the successful Yano package update. `yano update --check` remains read-only and does not run any update command. `yano uninstall` removes both the same way, asking a separate confirmation for the second one.

The package step has a hard post-install gate: `yano update` exits non-zero unless npm has produced a real global directory with a readable version. Therefore `npm ls -g yano-orchestrator --depth=0` must show no `->` after a successful update. The source is the repository URL in the installed package (normally GitHub); uncommitted changes in a development checkout are not promoted implicitly.
...
# il daemon Docker ha un comando di avvio noto per OS (yano init/doctor lo tenta già da soli); registralo per farlo riavviare anche ogni minuto dal cron
yano services add --name docker --healthcheck-command "docker info" --restart-command "systemctl start docker || service docker start"  # macOS: open -a Docker
