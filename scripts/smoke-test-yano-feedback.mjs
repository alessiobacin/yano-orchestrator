...
const { openDatabase, createFeedback, claimFeedback, listFeedback, repairFeedbackScreenshots, updateFeedback } = await import("./yano-feedback.mjs");
...
const moved = await updateFeedback(db, first.id, { status: "paused", audit_reason: "smoke: pausa manuale" });
assert.equal(moved.status, "paused", "a status-only dashboard update must not be rewritten to queued");
const repaired = await repairFeedbackScreenshots(db, { ...bug, screenshots: [{ url: "https://127.0.0.1:1/missing.png" }] });
assert.equal(repaired.screenshots.length, 1, "transient screenshot network failure must preserve the URL");
