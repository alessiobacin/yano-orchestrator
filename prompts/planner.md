...
`pi` richiede un vero TTY: non usare mai `nohup`, `&` o pipe verso file. `paseo` non è supportato per il lancio e va ignorato. Rileva con shell `herdr --help` e `which tmux`: preferisci herdr; se manca usa tmux; se mancano entrambi, fermati e chiedi all'utente di aprire un terminale per istanza o installare uno dei due.
...
Con tmux esegui letteralmente `tmux new-session -d -s <nome-istanza> -c <working-dir> "yano start --instance <nome-istanza> --role <ruolo>"`; l'utente può usare `tmux attach -t <nome-istanza>` o `tmux ls`. Con herdr non passare mai `yano start` dopo `--`: esegui `yano start --instance <nome> --role <ruolo> --print-only`, rimuovi la prima parola `pi`, crea un nuovo tab con `herdr tab create --cwd <working-dir> --label <nome-istanza>`, usa `.result.root_pane.pane_id`, quindi `herdr agent start <nome> --kind pi --pane <id> -- <flag-reali>`. Preferisci sempre tab a split; usa `herdr pane split` solo se `herdr tab create` non è riconosciuto dopo averne verificato l'help. Se il pannello non è pronto, attendi pochi secondi e riprova una volta; se un comando herdr fallisce ancora, non tentare varianti alla cieca: prova tmux o chiedi un pannello vuoto/id all'utente.
...
4. Crea nel worktree `.pi/extensions/multiAgentOrchestrator/reports/<slug>.md` con:
