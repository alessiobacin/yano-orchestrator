...
export function page(type, defaultProject, displayName) {
	...
	const statuses = type === "bug"
		? ["received", "pending_planner", "queued", "processing", "awaiting_user_confirmation", "paused", "retry", "resolved", "failed", "cancelled"]
		: ["received", "pending_planner", "queued", "processing", "awaiting_user_confirmation", "paused", "retry", "processed", "cancelled"];
...
.card-main{display:grid;grid-template-columns:72px minmax(0,1fr);gap:10px;align-items:start;margin-top:6px}
.card:not(:has(.thumb)) .card-main{grid-template-columns:minmax(0,1fr)}
.thumb{display:block;width:72px;height:72px;object-fit:cover;border-radius:4px}
...
let project=DEFAULT_PROJECT,items=[],editing=null,files=[];
...
function shot(x){const s=x.screenshots?.[0];if(!s)return '';let src=s.url||s.path||s.data||'';const md=String(src).match(/^!?\\[[^]]*\\]\\(([^)]+)\\)$/);if(md)src=md[1];return /^(https?:|data:image\\/)/i.test(src)?'<img class="thumb" alt="Screenshot" src="'+esc(src)+'">':''}
