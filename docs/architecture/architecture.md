...
`yano-local-pc` è un agente globale sempre attivo, che porta
...
9. If the task involved frontend roles, the planner asks whether the user wants a visual development review. On consent, `yano frontend-review start` installs the required development dependencies (React adapter or Angular host adapter), or — for Streamlit/Python/static frontends — runs a browser-only review through a Yano-served wrapper that forwards annotations to the project's agentation webhook without installing anything or touching app source; it infers and starts the frontend URL, and the planner consumes Agentation annotations through MCP.
